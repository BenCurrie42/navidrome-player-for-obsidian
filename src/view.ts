...
const TABBAR_COLLAPSE_BREAKPOINT = 220;
...
export class NavidromeView extends ItemView {
  ...
  constructor(
    leaf: WorkspaceLeaf,
    private plugin: NavidromePlugin,
  ) {
    super(leaf);
  }
  ...
  async onOpen(): Promise<void> {
    ...
    this.tabSelect = seg.createEl("select", {
      cls: "dropdown navidrome-tab-select",
    });
    ...
    this.tabSelect.onchange = () =>
      this.switchTab(this.tabSelect.value as TabId);
  ...
  private buildSearchBar(container: HTMLElement) {
    ...
    this.searchInput = wrap.createEl("input", {
      cls: "navidrome-search-input",
    });
    this.searchInput.type = "search";
    this.searchInput.placeholder = "Search artists, albums, songs…";

    this.searchClearBtn = wrap.createEl("button", {
      cls: "navidrome-search-clear",
    });
